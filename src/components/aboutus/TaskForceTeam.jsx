...
const FacultyMembers = () => {
  ...
  return (
    <KiteBackground>
      <div
        className="pt-12 px-9 text-white flex bg-none flex-col items-center pb-16 z-20"
      >
        ...
        <div className="w-full max-w-7xl mx-auto space-y-10">
          {groupedData.map((section, index) => (
            <div
              key={index}
              className={`flex flex-col sm:flex-row border border-white rounded-xl overflow-hidden min-h-auto
                ${index === 3
            ...
            >
              ...
              <div className={`flex overflow-x-auto gap-4 justify-between px-1 w-full`}>
                {section.members.map((member, i) => (
                  <div
                    key={i}
                    className={`  rounded-br-[14.21px] rounded-bl-[14.21px] overflow-hidden w-[140px] sm:w-[180px] md:w-[220px] xl:w-[250px] flex-shrink-0
                      ${i === 0 ? "-translate-x-1" 
                        : i === (section.members.length - 1) ? "translate-x-1" 
                        : "translate-x-0"}
  ...
  return (
    <div className="flex flex-col h-auto">
      <img
        src={member.image}
        alt={member.name}
        className="w-full h-[220px] sm:h-[260px] md:h-[280px] object-cover border- border-white"
      />
      <div className="bg-white text-black text-center py-1 sm:py-2 px-1 sm:px-2 w-full rounded-bl-[0px] rounded-br-[14.21px] flex-1">
