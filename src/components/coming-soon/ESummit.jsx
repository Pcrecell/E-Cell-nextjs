  ...
  const gradientRectangles = [0, 1, 2, 3, 4, 5].map((index) => (
    <motion.div
      ...
      style={{
        left: `${(index * 100) / 6}%`,
        width: `${100 / 6}%`,
        background: `linear-gradient(to bottom, #00817A 0%, #000000 100%)`
      }}
...
export const LampContainer = ({ className, navigate }) => {
  return (
    ...
    >
      {/* Lamp glow effect with matching teal theme */}
      <div className="absolute scale-x-50 md:scale-x-100 inset-0 flex items-center justify-center z-10 pointer-events-none">
        <div className="relative w-full h-[500px] flex items-center justify-center">
          <motion.div
            ...
            className="absolute right-1/2 h-56 w-[30rem] bg-gradient-conic from-[#00817A] via-transparent to-transparent text-white [--conic-position:from_70deg_at_center_top]"
          >
          ...
          <motion.div
            ...
            className="absolute left-1/2 h-56 w-[30rem] bg-gradient-conic from-transparent via-transparent to-[#00817A] text-white [--conic-position:from_290deg_at_center_top]"
          >
          ...
          {/* Extra glow layers with matching teal color */}
          <div className="absolute top-1/2 h-48 w-full translate-y-12 scale-x-150 bg-slate-950 blur-2xl" />
          <div className="absolute top-1/2 z-50 h-48 w-full bg-transparent opacity-10 backdrop-blur-md" />
          <div className="absolute z-50 h-36 w-[28rem] -translate-y-1/2 rounded-full bg-[#00817A] opacity-50 blur-3xl" />
          <motion.div
            initial={{ width: "8rem" }}
            whileInView={{ width: "16rem" }}
            transition={{ delay: 2.0, duration: 0.8, ease: "easeInOut" }}
            className="absolute z-30 h-36 w-64 -translate-y-[6rem] rounded-full bg-[#00817A] blur-2xl"
          />
          <motion.div
            initial={{ width: "15rem" }}
            whileInView={{ width: "30rem" }}
            transition={{ delay: 2.0, duration: 0.8, ease: "easeInOut" }}
            className="absolute z-50 h-0.5 w-[30rem] -translate-y-[7rem] bg-[#00817A]"
          />
