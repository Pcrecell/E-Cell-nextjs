  ...
  const gradientRectangles = [0, 1, 2, 3, 4, 5].map((index) => (
    <motion.div
      ...
      style={{
        left: `${(index * 100) / 6}%`,
        width: `${100 / 6}%`,
        background: `linear-gradient(to bottom, #FF6F15 0%, #000000 100%)`
      }}
    ...
    >
      <div className="absolute scale-x-50 md:scale-x-100 inset-0 flex items-center justify-center z-10 pointer-events-none">
        <div className="relative w-full h-[500px] flex items-center justify-center">
          <motion.div
            ...
            className="absolute right-1/2 h-56 w-[30rem] bg-gradient-conic from-[#FF6F15] via-transparent to-transparent text-white [--conic-position:from_70deg_at_center_top]"
          >
          ...
          <motion.div
            ...
            className="absolute left-1/2 h-56 w-[30rem] bg-gradient-conic from-transparent via-transparent to-[#FF6F15] text-white [--conic-position:from_290deg_at_center_top]"
          >
          ...
          {/* Extra glow layers with matching #FF6F15 color */}
          <div className="absolute top-1/2 h-48 w-full translate-y-12 scale-x-150 bg-slate-950 blur-2xl" />
          <div className="absolute top-1/2 z-50 h-48 w-full bg-transparent opacity-10 backdrop-blur-md" />
          <div className="absolute z-50 h-36 w-[28rem] -translate-y-1/2 rounded-full bg-[#FF6F15] opacity-50 blur-3xl" />
          <motion.div
            initial={{ width: "8rem" }}
            whileInView={{ width: "16rem" }}
            transition={{ delay: 2.0, duration: 0.8, ease: "easeInOut" }}
            className="absolute z-30 h-36 w-64 -translate-y-[6rem] rounded-full bg-[#FF6F15] blur-2xl"
          />
          <motion.div
            initial={{ width: "15rem" }}
            whileInView={{ width: "30rem" }}
            transition={{ delay: 2.0, duration: 0.8, ease: "easeInOut" }}
            className="absolute z-50 h-0.5 w-[30rem] -translate-y-[7rem] bg-[#FF6F15]"
          />
      ...
      <div className="relative z-50 flex flex-col items-center px-5 w-full">
        ...
        <div className="flex flex-col items-center mt-20 -translate-y-16">
          {/* Animated Logo */}
          <motion.img
            src={"https://ik.imagekit.io/fhervghik/E-Cell%20Website/Mavericks%20coming%20soon.png"}
            alt="Maverick Logo"
