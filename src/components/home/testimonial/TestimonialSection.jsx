...
const TestimonialSection = () => {
  ...
  // Card-specific arrow positioning - custom positions for specific cards
  const getArrowPosition = (currentIndex) => {
    // Specific positioning for each card by index
    const positions = {
      0: "bottom-20", // Card 1 - lower position
      1: "bottom-24", // Card 2 - lower position
      2: "bottom-20", // Card 3 - standard position
      3: "bottom-44", // Card 4 - lower position
      4: "bottom-40", // Card 5 - standard position
      5: "bottom-36", // Card 6 - lower position
      6: "bottom-28",
      7: "bottom-24",
      8: "bottom-36",
      // Add more positions as needed
    };
    return positions[currentIndex] || "bottom-16"; // default fallback
  };

  // Alternative: Dynamic positioning based on actual card height (advanced)
  const [arrowPosition, setArrowPosition] = useState("bottom-16");

  useEffect(() => {
    // Dynamically calculate arrow position based on card content
    const updateArrowPosition = () => {
      const currentCard =
        slideContainerRef.current?.querySelector(".testimonial-card");
      if (currentCard) {
        const cardHeight = currentCard.offsetHeight;
        const textContent = currentCard.querySelector(".text-content");
        const authorContent = currentCard.querySelector(".author-content");

        if (textContent && authorContent) {
          const authorHeight = authorContent.offsetHeight;
          const paddingBottom = 24; // Approximate padding
          const arrowPixels = cardHeight - authorHeight - paddingBottom;
          setArrowPosition(`${arrowPixels}px`);
        }
      }
    };

    // Update when slide changes or on resize
    if (isMobileOrTablet) {
      updateArrowPosition();
      const resizeObserver = new ResizeObserver(updateArrowPosition);
      const currentContainer = slideContainerRef.current;
      if (currentContainer) {
  ...
  return (
    <section className="relative w-full py-8 md:py-16 lg:py-24 lg:min-h-screen overflow-hidden">
      ...
      <div className="z-20 relative">
        <h2 className="text-4xl md:text-6xl lg:text-8xl text-white font-[Transcity] text-center z-20 mb-8 md:mb-12">
          Life At E-Cell
        </h2>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-2">
        {isMobileOrTablet ? (
          ...
          <div className="testimonial-carousel relative max-w-2xl mx-auto mb-4">
            ...
            {/* Navigation Arrows with Card-Specific Positioning */}
            <button
              onClick={prevSlide}
              className={`nav-button absolute left-4 ${getArrowPosition(
                currentSlide
              )} bg-black/70 hover:bg-black/90 text-white p-3 rounded-full transition-all duration-200 z-10 backdrop-blur-sm border border-gray-600/30`}
            ...
            <button
              onClick={nextSlide}
              className={`nav-button absolute right-4 ${getArrowPosition(
                currentSlide
              )} bg-black/70 hover:bg-black/90 text-white p-3 rounded-full transition-all duration-200 z-10 backdrop-blur-sm border border-gray-600/30`}
            ...
            {/* Alternative: Using dynamic pixel positioning (uncomment to use) */}
            {/*
            <button
              onClick={prevSlide}
              className="nav-button absolute left-4 bg-black/70 hover:bg-black/90 text-white p-3 rounded-full transition-all duration-200 z-10 backdrop-blur-sm border border-gray-600/30"
            ...
            <button
              onClick={nextSlide}
              className="nav-button absolute right-4 bg-black/70 hover:bg-black/90 text-white p-3 rounded-full transition-all duration-200 z-10 backdrop-blur-sm border border-gray-600/30"
              style={{ bottom: arrowPosition }}
              aria-label="Next testimonial"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <>
            ...
            <div className="relative w-[130%] justify-self-center">
              ...
              >
                <div
                  ref={bottomRowRef}
                  className="flex gap-6 py-2 overflow-x-hidden"
                >
